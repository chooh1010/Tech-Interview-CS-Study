...
<li> 동적 배열(ex. Java의 ArrayList)은 일반 배열, 링크드 리스트와 각각 어떤 차이를 갖나요?</li>
<li> 리스트에서 인덱스가 아닌 "값"으로 데이터를 찾는다면, 시간복잡도는 어떻게 되나요?</li>
<li> 그 리스트가 정렬되어 있다는 조건이 붙는다면, 시간복잡도는 어떻게 달라질까요? 링크드 리스트여도 마찬가지일까요?</li>
